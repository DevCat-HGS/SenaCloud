...
const EditarInstructor: React.FC = () => {
  ...
  useEffect(() => {
    const fetchInstructor = async () => {
      ...
      try {
        ...
        if (isConnected && instructorSocketRef.current) {
          ...
          const handleInstructorData = (data: any) => {
            console.log('Datos de instructor recibidos:', data);
            instructorSocketRef.current?.off('instructor-data', handleInstructorData);
            instructorSocketRef.current?.off('error', handleError);
            
            if (data.rol !== 'Instructor') {
              setError('El usuario no es un instructor');
              setLoading(false);
              return;
            }
          ...
          const handleError = (errorData: any) => {
            console.error('Error al obtener instructor:', errorData);
            instructorSocketRef.current?.off('instructor-data', handleInstructorData);
            instructorSocketRef.current?.off('error', handleError);
            setError(errorData.message || 'Error al cargar los datos del instructor');
            setLoading(false);
          };
          
          // Registrar listeners
          instructorSocketRef.current.on('instructor-data', handleInstructorData);
          instructorSocketRef.current.on('error', handleError);
          
          // Establecer un timeout por si no hay respuesta
          const timeout = setTimeout(() => {
            console.warn('Timeout al obtener instructor, usando fetch como fallback');
            instructorSocketRef.current?.off('instructor-data', handleInstructorData);
            instructorSocketRef.current?.off('error', handleError);
            // Fallback a HTTP
            fetchInstructorHttp();
          ...
          // Emitir evento para obtener instructor
          instructorSocketRef.current.emit('get-instructor', { id });
          
          return () => clearTimeout(timeout);
        } else {
          // Usar HTTP como fallback
          await fetchInstructorHttp();
        }
      } catch (err) {
    ...
    // Función para obtener instructor usando HTTP como fallback
    const fetchInstructorHttp = async () => {
      try {
      ...
      if (isConnected && instructorSocketRef.current) {
        return new Promise<void>((resolve, reject) => {
          ...
          const handleSuccess = (data: any) => {
            console.log('Instructor actualizado correctamente:', data);
            instructorSocketRef.current?.off('instructor-updated', handleSuccess);
            instructorSocketRef.current?.off('error', handleError);
            setSaving(false);
            navigate(`/Admin/usuarios/instructores/${id}`);
            resolve();
          };
          
          const handleError = (errorData: any) => {
            console.error('Error al actualizar instructor:', errorData);
            instructorSocketRef.current?.off('instructor-updated', handleSuccess);
            instructorSocketRef.current?.off('error', handleError);
            setError(errorData.message || 'Error al actualizar el instructor');
            setSaving(false);
            reject(new Error(errorData.message || 'Error al actualizar el instructor'));
          };
          
          // Registrar listeners
          if (instructorSocketRef.current) {
            instructorSocketRef.current.on('instructor-updated', handleSuccess);
            instructorSocketRef.current.on('error', handleError);
            
            // Establecer un timeout por si no hay respuesta
            const timeout = setTimeout(() => {
              console.warn('Timeout al actualizar instructor, usando fetch como fallback');
              instructorSocketRef.current?.off('instructor-updated', handleSuccess);
              instructorSocketRef.current?.off('error', handleError);
              // Fallback a HTTP
              updateInstructorHttp(instructorData)
                .then(resolve)
                .catch(reject);
            }, 5000); // 5 segundos de timeout
            
            // Emitir evento para actualizar instructor
            instructorSocketRef.current.emit('update-instructor', { id, ...instructorData });
          } else {
        ...
        // Usar HTTP como fallback
        await updateInstructorHttp(instructorData);
        navigate(`/Admin/usuarios/instructores/${id}`);
      }
    } catch (err) {
      console.error('Error:', err);
      setError(err instanceof Error ? err.message : 'Error al actualizar el instructor');
    } finally {
      setSaving(false);
    }
  };
  
  // Función para actualizar instructor usando HTTP como fallback
  const updateInstructorHttp = async (instructorData: any) => {
    const response = await fetch(`http://localhost:3001/api/users/${id}`, {
      method: 'PUT',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.message || 'Error al actualizar el instructor');
    }
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <Link
          to={`/Admin/usuarios/instructores/${id}`}
          className="flex items-center text-blue-600 hover:text-blue-800"
        >
          <ArrowLeft size={16} className="mr-1" /> Volver a detalles
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">Editar Instructor</h1>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label htmlFor="estadoInstructor" className="block text-sm font-medium text-gray-700 mb-1">
                Estado del instructor *
              </label>
              <select
                id="estadoInstructor"
          ...
          <div className="mt-8 flex justify-end">
            ...
            <button
              type="submit"
              disabled={saving}
              className={`px-4 py-2 bg-green-600 text-white rounded-md flex items-center ${saving ? 'opacity-70 cursor-not-allowed' : 'hover:bg-green-700'}`}
            >
              {saving ? (
                <>
